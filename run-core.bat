...
echo ===================================================
echo   Synapse mini - deterministic plan check
echo ===================================================
...
"%PYTHON_EXE%" --version 2^>^&1 | findstr /C:"Python 3.12." >nul
if errorlevel 1 goto setup_error
"%PYTHON_EXE%" "%SYNAPSE_ROOT%\_mini\synapse_mini.py" plan "%SYNAPSE_ROOT%\_mini\examples\local_docs_search.md" --preset software
set "SYNAPSE_EXIT=%ERRORLEVEL%"
if not "%SYNAPSE_EXIT%"=="0" echo [ERROR] Deterministic plan stopped with code %SYNAPSE_EXIT%.
goto finish
